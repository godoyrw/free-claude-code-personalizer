...
async function load() {
  showMessage("Carregando configuração do admin");
...
function renderProviders(providerStatus) {
  ...
  providerStatus.forEach((provider) => {
    ...
    meta.textContent =
      provider.kind === "local"
        ? provider.base_url || "Nenhuma URL local configurada"
        : provider.credential_env;

    const button = document.createElement("button");
    button.type = "button";
    button.className = "test-button";
    button.textContent = provider.kind === "local" ? "Testar" : "Atualizar modelos";
  ...
  VIEW_GROUPS.forEach((view) => {
    ...
    view.sections.forEach((sectionId) => {
      ...
      if (sectionFields.some((field) => field.advanced)) {
        ...
        toggle.textContent = "Mostrar avançado";
        toggle.addEventListener("click", () => {
          const showing = sectionEl.classList.toggle("show-advanced");
          toggle.textContent = showing ? "Ocultar avançado" : "Mostrar avançado";
        });
...
function inputForField(field) {
  ...
  if (field.type === "secret") {
    ...
    input.placeholder = field.configured
      ? "Configurado - insira um novo valor para substituir"
      : "Não configurado";
    input.value = "";
...
function updateDirtyState() {
  ...
  byId("dirtyState").textContent =
    count === 0 ? "Sem alterações" : `${count} alteração${count === 1 ? "" : "ões"} não salva${count === 1 ? "" : "s"}`;
...
function showValidationResult(result) {
  if (result.valid) {
    showMessage("A configuração é válida", "ok");
  } else {
...
async function apply() {
  ...
  if (restart.required && restart.automatic) {
    showMessage("Aplicado. Reiniciando servidor...", "ok");
  ...
  showMessage(
    pending.length
      ? `Aplicado. Reinicie o fcc-server para usar: ${pending.join(", ")}`
      : "Aplicado",
    "ok",
  );
}
...
async function refreshLocalStatus() {
  ...
  result.providers.forEach((provider) => {
    ...
    const meta = provider.status_code
      ? `${provider.base_url} retornou HTTP ${provider.status_code}`
      : provider.base_url;
  ...
  try {
    ...
    if (result.ok) {
      updateProviderCard(
        providerId,
        "reachable",
        `${result.models.length} modelos`,
        result.models.slice(0, 3).join(", ") || "Nenhum modelo retornado",
      );
